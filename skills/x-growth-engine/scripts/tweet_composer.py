...
def validate_tweet(text: str) -> TweetDraft:
    ...
    # Links: no ranking penalty exists in X's open-source For You code (open_link weight
    # is +0.2). Note the tradeoff of the common "link in the reply" workaround instead.
    import re
    if re.search(r'https?://\S+', text):
        warnings.append("Contains URL — no ranking penalty in the published algorithm; note that "
                        "moving it to your own reply costs out-of-network reach, since replies "
                        "are dropped from For You for non-followers")

    # Hashtags: style only. No hashtag handling appears anywhere in the published algorithm.
    hashtags = re.findall(r'#\w+', text)
    if len(hashtags) > 2:
        warnings.append(f"Has {len(hashtags)} hashtags — reads as spammy to humans; no known "
                        f"algorithmic penalty")
    elif len(hashtags) > 0:
        warnings.append(f"Has {len(hashtags)} hashtag(s) — style choice, not a ranking factor")
